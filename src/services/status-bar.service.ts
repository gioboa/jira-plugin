...
export class StatusBarService {
  ...
  constructor() {
    ...
    this.awayTimeout = parseInt(services.configuration.getConfigurationByKey(CONFIG.TRACKING_TIME_MODE_HYBRID_TIMEOUT) || '30', 10) * 60;
  }
  ...
  public async updateWorkingProjectItem(project: string): Promise<void> {
    ...
    if (!project) {
      project = (await services.configuration.getConfigurationByKey(CONFIG.WORKING_PROJECT)) || '';
    }
    ...
    if (services.configuration.getConfigurationByKey(CONFIG.ENABLE_WORKING_ISSUE)) {
  ...
  private workingIssueItemText(workingIssue: IWorkingIssue): string {
    return workingIssue.issue.key !== NO_WORKING_ISSUE.key
      ? `Working Issue: ${workingIssue.issue.key || ''} $(watch) ${services.utilities.secondsToHHMMSS(workingIssue.trackingTime) || ''}` +
          (workingIssue.awayTime === 0
            ? ``
            : workingIssue.awayTime > 0
            ? ` $(history) ${services.utilities.secondsToHHMMSS(this.awayTimeout - workingIssue.awayTime)}`
  ...
  public updateWorkingIssueItem(checkGlobalStore: boolean): void {
    ...
    if (checkGlobalStore) {
      issue = services.configuration.getGlobalWorkingIssue(state.context);
      if (!!issue) {
        ...
        services.configuration.setGlobalWorkingIssue(state.context, undefined);
        return;
      }
    }
  ...
  public startWorkingIssueInterval(): void {
    ...
    this.intervalId = setInterval(() => {
      if (
        ...
        services.configuration.getConfigurationByKey(CONFIG.TRACKING_TIME_MODE) === TRACKING_TIME_MODE.ALWAYS
      ) {
        if (services.configuration.getConfigurationByKey(CONFIG.TRACKING_TIME_MODE) === TRACKING_TIME_MODE.HYBRID) {
      ...
      } else if (services.configuration.getConfigurationByKey(CONFIG.TRACKING_TIME_MODE) === TRACKING_TIME_MODE.HYBRID) {
